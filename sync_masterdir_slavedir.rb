...
class SyncDir
  def sync_src_with_dest(src_dir, dst_dir, force_dir_arr, exclude_in_src)
    ...
    copy_newfiles_to_dest(dst_dir, exclude_in_src, items_not_in_dst, src_dir)
    exclude_in_src << items_not_in_dst
    exclude_in_src.flatten!
    copy_files_to_dest(dst_dir, exclude_in_src, src_items, src_dir, force_dir_arr)
  end
...
if $0 == __FILE__
  src_dir = 'D:\tmp\test'
  dst_dir = 'C:\Program Files (x86)\apache-tomcat-7.0.57\webapps\Test'
  
  exclude_in_src = ['SepuraWebUI.csproj','SepuraWebUI.csproj.vspscc','app/config.json', 'packages.config']
  #force_dir = ['Scripts']
  force_dir = []
  sync = SyncDir.new
  sync.sync_src_with_dest(src_dir, dst_dir, force_dir, exclude_in_src)
end